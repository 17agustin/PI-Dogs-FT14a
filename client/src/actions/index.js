...
export function getAllDog (){
  return function(dispatch){
    return axios.get(`${BASE_URL}/dogs`) //body
...
export function getDogsByQuery (query) {
    return function(dispatch) { 
      return axios.get(`${BASE_URL}/dogs?name=${query}`)
        .then(response => response.data)
        .then(data => {
...
export function getDogDetail(id) {
  return function(dispatch) {
    return axios.get(`${BASE_URL}/dogs/${id}`)
      .then(response => response.data)
      .then(data => { 
        if(data.created){
          let send = {
            ...
            height: data.height,
            weight: data.weight,
            temperaments: data.temperaments.map(t => t.name)
          }       
...
export function getTemperaments() {
  return function(dispatch) {
    return axios.get(`${BASE_URL}/temperament`)
      .then(response => response.data)
      .then(data => { 
...
export function filterDogsN (dogs) {
  return function(dispatch){var send = dogs.filter(dog => dog.created === true)
...
const generaPesoProm = (weightA,weightB) =>{
  ...
  if(isNaN(parseInt(weight1[0]))) a = 18; else a=parseInt(weight1[0])
  if(isNaN(parseInt(weight2[0]))) c = 18; else c=parseInt(weight2[0])
  if(isNaN(parseInt(weight1[1]))) b = a; else b=parseInt(weight1[1])
  if(isNaN(parseInt(weight2[1]))) d = c; else d=parseInt(weight2[1])
  prom1= (a+b)/2;
  prom2= (c+d)/2;
  return {prom1,prom2}
}
...
export function filterTemp (dogs,temps){ 
  return function(dispatch){
      ...
      var filtered = dogs.filter((dog) => { 
        if(dog.temperaments && dog.temperaments.includes(t)) return dog
        else return null 
      })
